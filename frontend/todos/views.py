...
class TodoCreateView(AuthMixin, View):
    def post(self, request):
        ...
        if title:
            services.create_todo(None, title, due_date, due_time, start_date, description, priority, status, token=token)
...
class TodoUpdateView(AuthMixin, View):
    def post(self, request, todo_id):
        ...
        if todo and user and todo.get('user') and todo['user'].get('id') == user.get('id'):
            ...
            services.update_todo(todo_id, title=title, due_date=due_date, due_time=due_time, start_date=start_date, description=description, priority=priority, status=status, token=token)
        return redirect('todo-list')
